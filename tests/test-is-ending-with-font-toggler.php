...
/**
 * Test for Disable_Google_Fonts::is_ending_with_font_toggler().
 *
 * @since 2.0
 */
class FontTogglerCheckerTest extends WP_UnitTestCase {
	/**
	 * Test if strings that end with font toggler are marked so.
	 *
	 * @since 2.0
	 */
	public function test_strings_with_font_toggler() {
		...
		foreach ( $strings as $string ) {
			$this->assertTrue( $disable_google_fonts->is_ending_with_font_toggler( $string ) );
		}
	}

	/**
	 * Test if strings that do not end with font toggler are caught.
	 *
	 * @since 2.0
	 */
	public function test_strings_without_font_toggler() {
		$disable_google_fonts = new Disable_Google_Fonts();

		$strings = $disable_google_fonts->get_font_toggler_variants();

		foreach ( $strings as $string ) {
			$this->assertFalse( $disable_google_fonts->is_ending_with_font_toggler( $string ) );
		}
	}
}
